...
**"Read-only reconnaissance" is not an exemption.** `bash` / `pwsh` count as behavior-changing *unconditionally*, and under PTC the `run_code` call is the *carrier* of whatever it dispatches inside it — so an opening message that shells out to `git log`, `ls` or `pwd` owes the line exactly like a write does, in that same message. [observed in this deployment: denials land on *read-only* probes (`git log` / `ls` / `git status`), not on writes — one of them cost a full extra step.] To recon **without** owing a line, use the read-only tools (`read` / `grep` / `glob` / `lsp`) instead of the shell.
