...
- **改完不重挂就不生效 —— 而「开一个新会话」不等于「重挂」。** 配置（persona / `AGENTS.md` 段 / 插件行）在
  **挂载时**读取，而**同一个宿主进程里新开的会话加入既有挂载、不重读**。三次实测：
  ...
  ② 2026-09-20 10:28 **新开会话**（同进程，晚于文件 mtime 09:29 与提交 10:02）⇒ 读到的仍是 09:29 编辑**之前**的
  persona（日志里无 `user-opened turns` / `Phase 0.5`，却有更早那笔桶判据改版）—— 而仓库文件与部署软链都已是新版（`grep` 通过）；
  ③ **脚本**（reader 等不经挂载）⇒ 立即生效。
  ⇒ 判据：**要 persona / `AGENTS.md` / 插件行生效，必须有一次真正的重新挂载 —— 宿主进程重启（或让 preset 重新装配）**，
  而不是「开一个新会话」。旧写法「新会话是一次挂载」只对**新进程**成立，正是它让这条规则偶尔骗过你。
...
19. **「合规率」这一个词在本仓库有四个数（8% ~ 85%），差一个数量级的原因不是门在崩、也不是门很好
    ...
    - **「合规率」是机制健康检查，不是成绩**（2026-09-18 定）：它量的是门行**在不在、位置对不对**（内容质量要人读，
      #16），所以它答「机制有没有死」，不答「这个 preset 有没有价值」。另：**跨代次不可直接汇总** —— [实测] 2026-09-18：
      一场 0% 的会话（`session-380bfac8`）是 **pre-v3 代次**的产物（挂载早于 v3 判据落地，整场 1032 个事件里**零**看门狗
      注入 ⇒ 当时的判据认为它合规），把它与 v3 之后的会话合在一起算，得到的是一个没有意义的平均值。

    - **2026-09-19 v4：资格口径 = user-opened turns（授权边界制）—— 换的是**分母**，不是分子。** 起因：两场真实任务里
      闸门 **0 拒绝 / 0 RACE_REPORT**，而读者按「每轮」报 **5/9（56%）** ⇒ 同一门纪律出现**两份定义**：插件与 persona 按
      「真用户开的轮」要求门行（机器续行不欠），读者却把机器开轮的那些轮也算进分母。裁定（用户 2026-09-19）：采用
      **授权边界制**，插件逻辑**不动**，只改读者与 persona。
      - **新分母**：该轮起点被 claim 的那批 `user/message` 里、按 `seq` 最近于轮起点的那一条，其 `source.kind === 'user'`
        （其余 kind 一律机器开轮）。⚠️ 落盘形状是 `data.source.kind`（实测 386/386），**不是** `data.message.source.kind`；
        两条路径都读并把实际走哪条计数（漏兜底会让「全判机器开轮」静默发生）。
      - **新基线（口径 v4 · 窗口 = 两场真实任务 · 2026-09-19）**：

        | 会话 | 旧·每轮制（保留为对照） | 新·user-opened |
        |---|---|---|
        | pelican-bike `session-9a551078` | 3/5 (60%) | **3/3 (100%)** |
        | uh1h-huey `session-aef6f35a` | 2/4 (50%) | **2/2 (100%)** |
        | 合计 | 5/9 (56%) | **5/5 (100%)** |

      - **`user` 通道构成（同一窗口，权威字段 `source.kind`）**：pelican 53 条 = 真用户 **3** + 机器 **50（94%）**；
        uh1h 36 = 真用户 **2** + 机器 **34（94%）**。⇒ **按文本前缀分类会高估「真用户」**（我先前按前缀数到 5 / 4，
        权威判据是 `source.kind`）；这条正是 #24 那条「临时统计只准提假设」的又一实例。
      - 旧「每轮制」的四个对照读数（① 存在 / ② 前置 / 可见回复 / 任意文本）**一个没删**，标为「对照口径（每轮制，旧读数）」；
        机器开轮的**数量**与其中**动了手的轮数**只作观察项，不进分子分母。
    - **读法注记（2026-09-20）：被闸门拦过、随后补进门行再重发的轮次，在 ① 口径下仍记 ✗** —— 别读成「模型无视门线」。
      实例 `session-06c26b0c` 的 T2：user-opened 轮，第一步没写门行就动手 ⇒ 闸门拒了**一次**（`tool/result` @seq 388，
      turn/step = 2/1，理由即 `[intent-gate]` 那段原文），模型第二步补写门行并重发成功；读者仍记 `T2[user]✗[动后补]`
      （门行不在载体消息里）。⇒ **闸门修的是行为，① 量的是位置**，两者可以同时为真；不新增「闸门纠正过」这一类
      （那只会粉饰数字），只记读法。（拒绝在日志里的**落点形状**见 #23 的 2026-09-21 更正 —— 上面那个
      `tool/result` 是当时按 native 形状记的。）

    - **2026-09-21 追加：闸门拒绝的读数口径 —— enforce 之后 ① 不再等于「模型记没记住」。** 开闸后
      「① 存在」由闸门兜底，于是它量的是「闸门拦没拦住」，不是「模型有没有自觉」；能反映纪律成本的
      量变成**拒绝次数与拒完有没有恢复**。口径（**唯一登记处就是本条**；reader 与插件判据同源）：
      · **成对判**：`name` 命中 BEHAVIOR_TOOLS **且**结果文本含 `[intent-gate]` —— **不看参数**
        （参数里提到它的是「谈论」，见 #23 的更正）；PTC 落点是 `tool/ptc-dispatch` + `isError: true`，
        native 落点是 `tool/result`（名字靠 callId 回查 `tool/call`）。
      · 逐场三个数：**拒绝 n 次 / 恢复 m / 无门行继续 k**。恢复 = 门行在**被拒那次调用之后**才出现；
        无门行继续 = 被拒之后仍有行为动作落在门行之前（含整轮没补）—— 那是「每轮最多拒一次」的代价面。
      · **假拒候选** = 被拒过的轮**最终仍判 ① 合规**（`denied && ① 合规`）：enforce 版的竞态签名，
        与插件的 `FALSE_DENY` 注入同一判据，也是 `docs/decisions/0003` 的**回滚触发条件**。
      · 这些读数只作**观察项**，不接退出码（模型行为不该让套件随机变红，同本条既有取舍）。
...
22. **token / 成本读数的口径 —— 数据早在盘上，但「求和」必须照抄框架口径，自造 `sum()` 会算错。**
    ...
    - **图像 / 附件的 token 定价住在路由适配器里，不在框架里**：框架只做 surface 组装并把每个 surface 交给
      `LlmImageRequestPricing.priceImages()`（`token-meter/src/route-pricing.ts:2-3,47`）；**没有 route pricing 时图像只贡献
      `imageStructuralTokens` 占位**。本部署的实现是 `@mars-sea/dsh-commandcode-provider`（`lib/index.js:3640` → `priceRequestImage`，
      **只吃 `(width,height)`、不吃真图字节**）⇒ 算图像份额**必须调路由适配器**，别自己推公式。
    - ⇒ **图像的「字节账」换不成 token**（非线性）：可随时用适配器复算 —— [实测] 2026-09-20 `1280x800→642` / `1280x720→578` /
      `1246x1620→1010` / `946x760→458`，另加 `estimateContent(空文本)=4` 才是 surface 值。**这是框架行为（可复算），不是会话统计**
      —— 与 #24 的「探针级」不同，它允许进文档。
...
23. **读「某机制到底注入过没有」必须用注入前缀 + 阳性对照 —— 子串扫会话日志是**假阳性制造机**。**
    ...
    **无一条**是插件注入 —— 全部是引用：插件源码（`const RACE_REPORT = [`）、#19 的正文、模型自己的
    ...
    - **判别读法**：扫**注入前缀**并按**事件类型分组** —— 注入只可能落在 `agent/inbox/spliced` 或
      ...
      里，就是**谈论**它，不是**发生**它。**但注入通道里的命中也可能是转发**：续跑 checkpoint
      会把历史的门行原文搬进 `agent/inbox/spliced` / `user/message`（[实测] 2026-09-18）⇒ 命中之后还要再看一眼
      它是**摘要 / 转发**，还是**发生**。
    ...
    - **推广**：任何「这机制历史上触发过吗」的问题，先问三件事 —— 它的触发形态是什么字面、走哪条通道、
      阳性对照是什么 —— 再扫描。
    - **否定证据不走注入通道**：闸门在 `enforce` 下拒绝时，落点是一条 `tool/result`（前缀 `[intent-gate]`，
      与提醒的 `[intent-gate-watchdog]` 是两个词）—— 拿注入前缀那套读法去扫拒绝，会把**真证据**过滤掉；反过来
      `tool/result` 里的命中也可能是**读插件源码**或复述 ⇒ 拒绝证据要成对判：**该调用是行为工具 + 结果是拒绝理由**。
      [实测] 2026-09-18：全库字面命中里恰好 1 条是真拒绝（我刻意做的阳性对照）。
    - **2026-09-21 更正：PTC 下拒绝**不落 `tool/result`。**实测**（同一场 `session-5d9dcf8f`）：
      闸门拒绝落成一条 `tool/ptc-dispatch`，`isError === true`、`name` 就是被拒的行为工具名（`bash`）、
      文本 = `Error: [intent-gate] …`，且**没有 `turn` 字段**（要按 `rootCallId` 回指那一轮）。
      同场日志里还有一条 `isError === false` 的同名 `bash` —— 正是我自己跑 `grep '[intent-gate]'` 的那次，
      它的**参数**里也带着这个词 ⇒ 按「文本/整行含 `[intent-gate]`」计数会把它算成一次拒绝（假阳性）。
      ⇒ 判据固定为**成对**：名字在 BEHAVIOR_TOOLS **且**结果文本含 `[intent-gate]`（**不看参数**）；
      reader 的拒绝读数按此实现，并用 fixture 把「真拒绝」与「只提及」两条并排钉住。

    - **统计类读数也必须用机制自己的位置定义**：门行次数 / 桶分布只能用**消息首行**匹配
      （`^(Intent|意图判定):\s*<桶>\s*[—-]`）；子串扫会被**测试夹具里的引文**与**流式分片**灌成假阳性。
      [实测] 2026-09-18：一次子串扫给出 520 条「门行」，按首行重测真值是 **37** 条；我据此得出的
      「`investigation` 是死支」结论当天撤回。

24. **会话数据的结论必须由唯一 reader 出 —— 临时的子串扫 / 一次性脚本会给出「很像结论」的错数。**
    [实测] 2026-09-18 同一次分析里，我用临时脚本连错四次，而唯一 reader（`verify-ptc-roles.cjs`）的每个数字都对：

    | # | 临时统计写了什么 | 真值 / 错因 |
    |---|---|---|
    | ① | 子串扫「门行」得 **520 条**，并据此断言「`investigation` 是死支」 | 真值 **37**（差 14×）：扫到了**测试夹具里的引文**与**流式分片**；首行匹配后才是 37，结论当天撤回（#23） |
    | ② | 某工作区 `user/message` 计数 **0** | 提取器漏了 `data.content` / `data.text` 兜底 |
    | ③ | 每轮「门行 = 无」 | 取了**所有** part 的文本（含推理），首行自然不是 marker；应只取 `type === 'text'` 的 part |
    | ④ | 我自己的门行**两次不是首行** ⇒ 被闸门当场拒 | 机制没错，是我没按形状写（门行必须是那条消息的**首行**） |

    - **规则**：临时脚本只能**提出假设**；任何要进结论或文档的数字，必须由 reader 复核一次（`--cwd <path>` 可跨工作区）。
    - **与 #23 同源**：读会话日志的任何统计，都必须先把**位置定义**说清楚（哪一行 / 哪种事件类型 / 哪条通道），
      否则统计到的是「谈论它的文本」，不是「发生的事」。
    - **推论（同样适用于声明）**：没有留下**动作记录**的声明等于没发生 —— 「我已派了子代理」在数据面上必须能查到那次调用。
...
27. **persona 必须自包含 —— 它要在**任意工作区**运行，不要引用本仓相对路径，也不要假设某个 preset 已装。**
    [实测] 2026-09-21：薄版本与完整版两份 persona 都写着 `(docs/pitfalls.md #26)` —— 那是**本仓库**的路径；
    会话跑在别的项目里时，模型去读它会**读不到**（更糟的是它会把一次「读不到」当成一次失败）。
    同批修掉的还有 Upgrade hint：它建议「切到 `ptc-roles` preset」，而本机根本没装那个 preset
    —— 用户 2026-09-21 裁定「只装 `ptc-gate` 是有意的」。⇒ 两条纪律：
    ① persona 里只写**自包含**的规则（要引用本仓文件就展开成一句话）；
