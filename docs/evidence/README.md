...
**（2026-09-23 核对，前置已满足）**：闸门的观察模式取证与开闸 —— 门自 2026-09-21 起在 `enforce` 下运行，
模式标记与拒绝读数都已在数据面（ADR 0003 与证据 ⑮⑰⑱），旧版「必须新会话（`?v=10`）才吃得到新代」的前置早就满足；
若仍要单独取「观察模式」的证，把 `gate:` 临时改回 `observe` 再跑一轮即可（**不是**默认动作）。
